...
class RolSeed(BaseSeed):
    ...
    def up(self):
        ...
        # Crear índices
        index_sql = "CREATE INDEX IF NOT EXISTS idx_rols_name ON rols(name)"
        self.execute_sql(index_sql)

        
    def down(self):
        sql = "DROP TABLE IF EXISTS rols"
        self.execute_sql(sql)
    ...
    def seed(self):
        ...
        for name, activate, description, created_date, id_user_create in rols:
            try:
                sql = "INSERT INTO rols (name, activate, description, created_date, id_user_create) VALUES (?, ?, ?, ?, ?)"
